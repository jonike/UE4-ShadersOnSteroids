...
// Windows.h for correcting ShaderCompileWorkers priority on the run.
...
#include "HAL/ConsoleManager.h" // For handling read/write of console commands.
...
// Returns "wildmode" value for priority setting.
int32 PriorityModifierFromCommand()
{
	// Dirty implementation, registers wildmode variable on the go if it hasn't been created already.
	if (IConsoleManager::Get().FindConsoleVariable(TEXT("wildmode")) == NULL)
	{
		GLog->Log("Creating WILDMODE command for the first time");
		IConsoleManager::Get().RegisterConsoleVariable(TEXT("wildmode"), 0,
			TEXT("When on, sets subsequent ShaderCompileWorkers priority to High."),
			ECVF_Scalability | ECVF_RenderThreadSafe);
	}
	return IConsoleManager::Get().FindConsoleObject(TEXT("wildmode"))->AsVariableInt()->GetValueOnAnyThread();
}
...
#if PLATFORM_WINDOWS
					// Correct priority as requested from "wildmode" command variable when in between jobs.
					switch (PriorityModifierFromCommand()) {
					case 0: SetPriorityClass(CurrentWorkerInfo.WorkerProcess.Get(), DWORD(BELOW_NORMAL_PRIORITY_CLASS));
						break;
					case 1: SetPriorityClass(CurrentWorkerInfo.WorkerProcess.Get(), DWORD(HIGH_PRIORITY_CLASS));
						break;
					default: SetPriorityClass(CurrentWorkerInfo.WorkerProcess.Get(), DWORD(BELOW_NORMAL_PRIORITY_CLASS));
						break;
					}
#endif
...
FProcHandle FShaderCompilingManager::LaunchWorker(const FString& WorkingDirectory, uint32 InProcessId, uint32 ThreadId, const FString& WorkerInputFile, const FString& WorkerOutputFile)
{
	...
	// Priority for process from "wildmode" command.
	int32 PriorityModifier = PriorityModifierFromCommand();
